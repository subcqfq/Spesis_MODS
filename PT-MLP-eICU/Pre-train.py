...
# %%
# Check whether a GPU is available, otherwise use CPU
...
model_pre_path = './eicu_mimic_pre_dead_rate.ckpt'     # Path to save checkpoint (location used when saving the model)
...
def same_seeds(seed):  # Fix random seeds (CPU)
    ...
    np.random.seed(seed)  # Ensure random numbers generated by random functions are reproducible
    torch.backends.cudnn.benchmark = False  # Can be True when GPU and network structure are fixed
...
model = LR(x1_input_dim=7, x1_hidden_dim=8, x1_num_layers=8, x2_input_dim=34, x2_hidden_dim=64, x2_num_layers=10, hidden_dim=64, num_layers=8, output_dim=2).to(device)
