...
pre_model_path = './LSTM_mimic_pre_dead_rate.ckpt'
model_path = './LSTM_MIMIC_all_params.ckpt'     # Path to save model checkpoint
...
def same_seeds(seed): # Fix random seeds (CPU)
    ...
    np.random.seed(seed)  # Ensure reproducible random numbers
    torch.backends.cudnn.benchmark = False # For fixed GPU and model structure, can be True
...
class SepsisData(Dataset):
    def __init__(self, data, learn_w, lead_w, pred_w):
        ...
        for i in row_id:
            ...
            end_boundary = data_row.shape[0]  # If no positive label, boundary is entire table

            if (end_boundary <= learn_w):  # Skip patients with fewer samples than learning window
                continue
            ...
            for j in range(end_boundary - learn_w - lead_w):
                ...
                positive_flag_6 = 0
                for k in range(j + learn_w + lead_w, min(j + lead_w + learn_w + 6, data_row.shape[0])):  # Label positives within prediction window
                ...
                if (positive_flag_6 == 0):
                    label.append(0)
                positive_flag_12 = 0
                for k in range(j + learn_w + lead_w, min(j + lead_w + learn_w + 12, data_row.shape[0])):  # Label positives within prediction window
                    if (data_row.iloc[k, -1] == 1):
                        positive_flag_12 = 1
                        label.append(1)
                        break
                if (positive_flag_12 == 0):
                    label.append(0)
                positive_flag_24 = 0
                for k in range(j + learn_w + lead_w, min(j + lead_w + learn_w + 24, data_row.shape[0])):  # Label positives within prediction window
                    if (data_row.iloc[k, -1] == 1):
                        positive_flag_24 = 1
                        label.append(1)
                        break
...
for epoch in range(num_epoch):
    ...
    if mean_valid_auc > best_auc:
        ...
        torch.save(model.state_dict(), model_path) # Only save best to prevent memory overflow
...
# Assume these arrays have already been obtained in your test code
# test_prob_all_6, test_label_all_6, test_pred_all (binary classification results)
y_true = test_label_all_12
y_score = test_prob_all_12
...
y_pred = (y_score >= threshold).astype(int)
n_samples = len(y_true)
n_bootstraps = 1000  # Can be adjusted if needed
rng = np.random.RandomState(42)

# Used to store metrics for each bootstrap iteration
auc_list = []
acc_list = []
tpr_list = []
tnr_list = []
...
# Create a list of seeds as inputs for each task
...
# Plot ROC curves
plt.figure()
plt.plot(fpr_6, tpr_6, color='darkorange', lw=2, label=f'4 hours (AUC = {roc_auc_6:.3f})')
plt.plot(fpr_12, tpr_12, color='darkgreen', lw=2, label=f'8 hours (AUC = {roc_auc_12:.3f})')
plt.plot(fpr_24, tpr_24, color='darkblue', lw=2, label=f'12 hours (AUC = {roc_auc_24:.3f})')
