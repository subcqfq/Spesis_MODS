...
model_pre_path = './LSTM_mimic_pre_dead_rate.ckpt'  # Path to save model checkpoint

# %%
def same_seeds(seed):  # Fix random seed (CPU/GPU)
...
# %%
data = dd.concat([train_data, test_data, valid_data], axis=0)
data = data[data['stay_id'] > 30000000]
row_id = list(set(data['stay_id']))
train_id, test_id = train_test_split(row_id, test_size=test_ratio, random_state=seed)
train_id, valid_id = train_test_split(train_id, test_size=valid_ratio, random_state=seed)
train_data = data[data['stay_id'].isin(train_id)].compute()
valid_data = data[data['stay_id'].isin(valid_id)].compute()
test_data = data[data['stay_id'].isin(test_id)].compute()

# %%
train_data = dd.from_pandas(train_data, chunksize=10000)
valid_data = dd.from_pandas(valid_data, chunksize=10000)
test_data = dd.from_pandas(test_data, chunksize=10000)
...
# %%
train_loader = DataLoader(train_set, batch_size=1024, shuffle=True, pin_memory=True, num_workers=16)
test_loader = DataLoader(test_set, batch_size=1024, shuffle=True, pin_memory=True, num_workers=16)
valid_loader = DataLoader(valid_set, batch_size=1024, shuffle=True, pin_memory=True, num_workers=16)
...
for epoch in range(num_epoch):
    ...
    if valid_auc > best_auc:
        ...
        torch.save(model.state_dict(), model_pre_path)
        best_auc = valid_auc
        stale = 0
    else:
        stale += 1
